...
def compute_energy(pset,box_length):
    ...
    tot_potential = 0.0 
    #print(np.linalg.norm(pset.vel(0)))
    #mypos = pos[0]
    for a in range(1,natom):
        ...
        # v = np.linalg.norm(pset.vel(a))
        # tot_kinetic = tot_kinetic + 0.5*pset.mass()*v**2
...
def insttemp(pset):
    ...
    tot_kinetic = 0.5*pset.mass()*(vel**2).sum()
    inst = (2.0/3)*tot_kinetic/natom

    return (inst, tot_kinetic)
...
if __name__ == '__main__':
    ...
    for istep in range(nsteps):
        ...
        for iat in range(num_atoms):
            ...
            pset.change_accel(iat, internal_force(iat,pset,box_length)/pset.mass())
        # end for iat
